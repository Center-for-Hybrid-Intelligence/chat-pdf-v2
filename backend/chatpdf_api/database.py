...
class Namespace(db.Model):
    namespace_id = db.Column(db.DateTime(timezone=True),
                            server_default=func.now(), primary_key=True)
    namespace_name = db.Column(db.Text, primary_key=True)
...
def add_document(document_id, document_file, namespace_name):
    namespaces = Namespace.query.filter_by(namespace_name = namespace_name)
